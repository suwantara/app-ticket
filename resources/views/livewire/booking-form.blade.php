                ...
                @foreach ([1 => 'Kontak', 2 => 'Penumpang', 3 => 'Konfirmasi'] as $step => $label)
                    <div class="flex items-center">
                        <div class="flex flex-col items-center">
                            ...
                            <span
                                class="mt-2 text-sm {{ $currentStep >= $step ? 'text-blue-600 font-semibold' : 'text-gray-400' }}">
                                {{ $label }}
                            </span>
                        </div>
                        @if ($step < 3)
            ...
            <div class="lg:col-span-2">
                <div class="bg-white rounded-xl shadow-lg p-6">
                    ...
                    @if ($currentStep === 1)
                        @guest
                            <div
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h4 class="font-semibold text-blue-900">Pemesanan Lebih Cepat?</h4>
                                        <p class="text-sm text-blue-700">Login untuk isi data otomatis & simpan riwayat.</p>
                                    </div>
                                </div>
                                <a href="{{ route('login') }}"
                                    class="px-4 py-2 bg-blue-600 text-white text-sm font-bold rounded-lg hover:bg-blue-700 transition shadow-sm">
                                    Login
                                </a>
                            </div>
                        @endguest
                        ...
                        <p class="text-gray-500 mb-6">Data kontak akan digunakan untuk konfirmasi dan pengiriman
                            e-ticket.</p>

                        <div class="space-y-4">
                            <div>
                                <label for="contactName" class="block text-sm font-semibold text-gray-700 mb-2">Nama
                                    Lengkap</label>
                                <input type="text" id="contactName" wire:model="contactName"
                            ...
                            <div>
                                <label for="contactPhone" class="block text-sm font-semibold text-gray-700 mb-2">Nomor
                                    Telepon</label>
                                <input type="tel" id="contactPhone" wire:model="contactPhone"
                                    class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            <label for="passenger-id-type-{{ $index }}"
                                                class="block text-sm font-medium text-gray-700 mb-1">Jenis
                                                Identitas</label>
                                            <select id="passenger-id-type-{{ $index }}"
                                                wire:model="passengers.{{ $index }}.id_type"
                                                class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 cursor-pointer">
                    ...
                    @if ($currentStep === 3)
                        ...
                        <div class="mt-6 flex justify-between">
                            ...
                            <button x-data
                                ...
                                class="bg-linear-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white px-8 py-3 rounded-lg font-semibold transition-all shadow-lg cursor-pointer">
                        ...
                        <div class="border-b border-gray-200 pb-4 mb-4">
                            ...
                            <div class="text-sm space-y-1">
                                <div class="flex justify-between">
                                    ...
                                    <span
                                        class="font-medium">{{ \Carbon\Carbon::parse($travelDate)->translatedFormat('d M Y') }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-gray-500">Rute</span>
                                    <span class="font-medium">{{ $schedule->route->origin->name }} →
                                        {{ $schedule->route->destination->name }}</span>
                                </div>
                                <div class="flex justify-between">
                        ...
                        <div class="border-b border-gray-200 pb-4 mb-4">
                            ...
                            <div class="text-sm space-y-1">
                                <div class="flex justify-between">
                                    <span class="text-gray-500">Tanggal</span>
                                    <span
                                        class="font-medium">{{ \Carbon\Carbon::parse($returnDate)->translatedFormat('d M Y') }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-gray-500">Rute</span>
                                    <span class="font-medium">{{ $returnSchedule->route->origin->name }} →
                                        {{ $returnSchedule->route->destination->name }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-gray-500">Waktu</span>
                    ...
                    <div class="space-y-2 text-sm">
                        ...
                        @if ($returnSchedule)
                            <div class="flex justify-between">
                                ...
                                <span>Rp
                                    {{ number_format($returnSchedule->price * $passengerCount, 0, ',', '.') }}</span>
                            </div>
                        @endif
                    </div>
